...
const membersByWorkspaceId: Record<string, WorkspaceMember[]> = {
  ws_01: [
    {
      id: 'm_01',
      name: 'Alice Nguyen',
...
export function addWorkspaceMember(
  ...
  const list =
    membersByWorkspaceId[workspaceId] ?? membersByWorkspaceId.default!;
  membersByWorkspaceId[workspaceId] = [...list, member];
}
...
export function removeWorkspaceMember(
  ...
  const list =
    membersByWorkspaceId[workspaceId] ?? membersByWorkspaceId.default!;
  membersByWorkspaceId[workspaceId] = list.filter((m) => m.id !== memberId);
}
...
export function changeWorkspaceMemberRole(
  ...
  const list =
    membersByWorkspaceId[workspaceId] ?? membersByWorkspaceId.default!;
  membersByWorkspaceId[workspaceId] = list.map((m) =>
...
const recentActivityByWorkspaceId: Record<string, ActivityItem[]> = {
  ws_01: [
    {
      ...
      actor: 'Alice Nguyen',
      verb: 'invited',
      target: 'frank@acme.com',
      timestamp: '2026-02-18T09:14:00Z',
    },
    {
      id: 'a_02',
      actor: 'Carol Smith',
      ...
      timestamp: '2026-02-18T08:47:00Z',
    },
    {
      id: 'a_03',
      actor: 'Bob Martinez',
      verb: 'deployed',
      target: 'v2.4.1 → production',
    ...
    },
    {
      ...
      actor: 'David Lee',
      verb: 'completed_task',
      target: 'DB migration script',
      timestamp: '2026-02-17T16:30:00Z',
    },
    {
      id: 'a_05',
      actor: 'Alice Nguyen',
      verb: 'updated_settings',
      target: 'Data retention → 90d',
  ...
  ws_03: [
    ...
    },
    {
      ...
      actor: 'Karen White',
      verb: 'completed_task',
      target: 'CI pipeline fix',
      timestamp: '2026-02-18T06:15:00Z',
    },
    {
      id: 'a_11',
      actor: 'Leo Davis',
...
export function getRecentActivity(workspaceId: string): ActivityItem[] {
  return (
    recentActivityByWorkspaceId[workspaceId] ??
    recentActivityByWorkspaceId.default!
  );
}
...
export function getUsagePoints(workspaceId: string): UsagePoint[] {
  return (
    usagePointsByWorkspaceId[workspaceId] ?? usagePointsByWorkspaceId.default!
  );
}
